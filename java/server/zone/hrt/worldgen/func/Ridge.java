...
public record Ridge(DensityFunction temperature, DensityFunction vegetation) implements DensityFunction.SimpleFunction {
  ...
  private static final CubicSpline<Float, ToFloatFunction<Float>> buildSpline(float[] input) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      spline = spline.addPoint(start, sign * -0.75f, 0.5f);
      spline = spline.addPoint(point - RADIUS, sign * -0.05f, 0.5f);
      spline = spline.addPoint(point + RADIUS, sign * 0.05f, 0.5f);
      spline = spline.addPoint(end, sign * 0.75f, 0.5f);
    }
    return spline.build();
  }
  ...
  public double compute(DensityFunction.FunctionContext pos) {
    ...
    float tem = (float) this.temperature.compute(pos);
    float veg = (float) this.vegetation.compute(pos);

    return TEM_SPLINE.apply(tem) * VEG_SPLINE.apply(veg);
  }
